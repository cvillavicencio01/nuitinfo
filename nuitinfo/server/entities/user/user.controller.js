...
module.exports = function(userSchema) {
	...
	userSchema.statics.computeLogin = function(firstName, lastName, callback) {
		let login =
			firstName.substr(0, 1).toLowerCase() + '.' + lastName.substr(0, 9).toLocaleLowerCase();
		let lastLogin = login;
	...
	userSchema.statics.changeTeam = function(params, callback) {
		mongoose
			.model('User')
			.update({ _id: params.user._id }, { team: params.team._id }, (err) => {
				if (err) {
					return callback(err);
				}
	...
	function checkParametersExistsForCreate(req, res, callback) {
		...
		if (parametersAreGood && !req.body.login) {
			mongoose
				.model('User')
				.computeLogin(req.body.firstName, req.body.lastName, (err, login) => {
	...
	userSchema.statics.exGetAll = function(req, res) {
		mongoose.model('User').getAll({ isSystemAccount: { $ne: true } }, (err, users) => {
			if (err) {
				return Response.selectError(res, err);
			}
